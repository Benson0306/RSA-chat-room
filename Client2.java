...
public class Client2 extends JFrame implements ActionListener {  
 ...
 Client2 (){ 
        //設定及宣告視窗物件 
  //建立視窗JFrame
  super("RSA加密聊天室-Client2");          
  //用來放mane及ip--設定區域
  
  JPanel maneipPanel  = new JPanel();   
  //建來設定按鍵 
  JButton startexchang= new JButton("送出公鑰");
  maneipPanel.add(startexchang);
  startexchang.addActionListener(this);
    ...
    {   String temp=null;  
    	temp=message.substring(0,1);
    	 ...
    	 { 
             ...
             hispubk=RSA.getPubk(message.substring(1,message.length())) ; 
             incoming.append("已收到對方公鑰"+"\n"); 
    	 ...
    	 {
    		 hisplaintext=RSA.decrypt(message.substring(1,message.length()),pair.getPrivate());
   	   	   
             incoming.append(hisplaintext+"\n");   
 ...
 public void actionPerformed(ActionEvent e){
  ...
  //按下送出   
   ...
   {
    try{//送出資料
    	mycipher=RSA.encrypt(getDateTime()+name+":"+outgoing.getText(),hispubk);
        
   	    writer.println("&"+mycipher);   
        incoming.append((getDateTime()+name+":"+outgoing.getText())+"\n");
       //刷新該串流的緩衝。
         writer.flush();     
    }catch(Exception ex ){
	  ...
	  if((ip!=null))    
	   {
	    try{//送出資料
	    	...
	    	writer.println("%"+RSA.getStringPubk(pair)); 
	     //刷新該串流的緩衝。
	     writer.flush();         
	    }catch(Exception ex ){
